...
> ✅ 定调 2026-05-31(v3)。**角色锁定(见 ip-character.md),画风做成可选皮肤**。每次生图按文章调性选一种,或用户 `--style` 指定。默认 3D 盲盒。

## 通用约定(所有皮肤共享)

- 画幅:比例按内容逐张判断、公众号偏竖(见 SKILL.md「2.5 定比例」+ prompt-template 方向词联动表)。皮肤与比例正交,任何皮肤都能出 16:9/9:16/4:3/3:4/1:1
- 招牌:**红框圆眼镜 + "小互"名牌**任何皮肤都在
- **⛔ 表情随内容情绪变化**(核心,见 ip-character 映射表)
...
皮肤的实现就是一段英文风格 prompt + 适用场景说明,加新皮肤不用改代码。用户触发方式(对 Claude 说话即可):

- **有参考图**:"我想要这种画风"+ 上传几张喜欢的图。Claude 分析参考图的渲染方式/线条/配色/质感,提炼成一段风格 prompt(学原理不照抄:提"厚涂+低饱和+粗轮廓",不提原作者名)
- **只有描述**:"给我加一个吉卜力水彩感的皮肤"。Claude 直接按描述写风格 prompt

Claude 收到后的步骤:
1. 写出风格 prompt 段 + 适用场景一行,按上面皮肤 A-F 的格式追加到本文件,起一个字母编号
2. 用新皮肤 + 用户的 IP 锚点图生成 1-2 张基准图给用户看(传锚点,prompt 写"keep character, only change rendering style")
3. 用户认可才算入库;不认可就改 prompt 重生,⛔ 不能没验证就写进文件
4. 提醒用户:新皮肤同样要遵守「视觉契约六维」(背景/光影/景深/精致度/色调/角色),一篇文章内不混皮肤
