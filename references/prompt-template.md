...
每张图单独生成,不拼图。按正文内容替换变量。**画风段(STYLE_DNA / IP)从 style-dna.md、ip-character.md 注入——这两个定调前是占位,模板暂时跑不出图。**
...
模板第一句写成 `Generate one standalone {ASPECT} {ORIENTATION} ...`,YAML 头的 `aspect_ratio` 填同一个 `{ASPECT}` 值。比例按内容判断(见 SKILL.md 2.5),公众号场景无横向理由时偏竖 `3:4`,不要无脑套 16:9。
...
COLORS: 暖色橙红为主 + 语义色(红=警示/风险,青绿=正向/解决,灰蓝=低落,金=成功)。Color values and color names are rendering guidance only — do NOT display them as visible text in the image.
...
生图时传给生图途径的比例参数,必须和 prompt 第一句的 `{ASPECT} {ORIENTATION}` 同步——参数说竖版、prompt 说横版,出图会变形。
